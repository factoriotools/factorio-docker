...
* `--user` - (versions 0.16+) The uid/guid (in numerical form) on the host system to run the container as.  Defaults to uid:guid 845:845 - volumes must be owned by this user.
...
To keep things simple, the container uses a single volume mounted at `/factorio`. This volume stores configuration, mods, and saves.  Starting in version 0.16 Volume ownership must match the --user uid:guid used to start the container, or be set to 845:845 if using default settings.  To change ownership of existing volumes execute `chown -R 845:845 /path/to/existing/volume`.
